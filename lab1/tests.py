...
class TestShoppingMall(unittest.TestCase):
    ...
    def test_product_return(self):
        PurchaseOperation.make_purchase(self.customer, self.product, self.shop, 1, self.seller)
        success, message = ReturnOperation.return_product(self.customer, 0)
        self.assertTrue(success)
